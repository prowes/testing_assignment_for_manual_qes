...
def enter():
    ...
    return (f'<html><head><title>Calculatr</title></head><body><form action="" method="post" novalidate>{form.hidden_tag()}<p>{form.first_digit.label}<br>{form.first_digit(size=7)}</p><p>{form.second_digit.label}<br>{ form.second_digit(size=6)}<p>{form.action.label}<br>{form.action(size=6)}</p><p>{form.submit()}</p></form></body></html>')
